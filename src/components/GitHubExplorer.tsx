    ...
    return (
      <div
        className={`${styles.fileItem} ${
          selectedFile === node.path ? styles.active : ''
        }`}
...
export default function GitHubExplorer({ repoPath }: GitHubExplorerProps) {
  ...
  const handleFileClick = async (filePath: string) => {
    ...
    if (binaryExtensions.includes(ext)) {
      setFileContent('');
      setCodeError(
        `Preview not available for binary files (.${ext}). Please download it from GitHub.`
      );
      return;
    }

    if (codeCache[filePath]) {
      console.log(`[Cache] Memuat "${filePath}" dari state.`);
    ...
    try {
      ...
      if (localContent) {
        console.log(
          `[IndexedDB] Memuat "${filePath}" dari storage lokal.`
        );
      ...
      console.warn('Gagal akses IndexedDB, lanjut fetch manual.');
    }
